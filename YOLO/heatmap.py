...
def get_params():
    params = {
        'weight': 'E:/gaijin/shiyan/ultralytics-8.2.72/ultralytics-8.2.72/runs\detect/train2/weights/best.pt',  # 这选择想要热力可视化的模型权重路径
        'cfg': r'E:\gaijin\shiyan\ultralytics-8.2.72\ultralytics-8.2.72\ultralytics\cfg\datasets\hat.yaml',  # 这里选择与训练上面模型权重相对应的.yaml文件路径
...
if __name__ == '__main__':
    ...
    model(r'E:\gaijin\shiyan\ultralytics-8.2.72\ultralytics-8.2.72\23\1.jpg',
          'E:\gaijin\shiyan\ultralytics-8.2.72\ultralytics-8.2.72\23')  # 第一个参数是图片的路径，第二个参数是保存路径，比如是result的话，其会创建一个名字为result的文件夹，如果result文件夹不为空，其会先清空文件夹。